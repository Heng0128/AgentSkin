...
async function render(input, size, output) {
...
try {
  ...
  await render(source, 1024, png1024);
  await copyFile(png1024, path.join(assets, 'branding', 'icon.png'));
  ...
  for (const { size, file } of icoEntries) await render(source, size, file);
  ...
  for (const { size, file } of themeIcoEntries) await render(themeFileSource, size, file);
  await writeIco(themeIcoEntries, path.join(assets, 'branding', 'theme-file.ico'));

  await render(traySource, 18, path.join(runtimeAssets, 'trayTemplate.png'));
  await render(traySource, 36, path.join(runtimeAssets, 'trayTemplate@2x.png'));
  await render(source, 32, path.join(runtimeAssets, 'tray-icon.png'));
