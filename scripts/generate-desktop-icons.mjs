...
async function renderSVG(input, size) {
  ...
  return sharp(input).resize(size, size, { fit: 'contain', background: PNG_BG }).png().toBuffer();
}
...
try {
  ...
  const payload512 = await sharp(master).resize(512, 512, { fit: 'fill' }).png().toBuffer();
  ...
  const themeIcoEntries = icoSizes.map((size) => ({
    size,
    file: path.join(work, `theme-file-${size}.png`),
  }));
  ...
  await copyFile(
    path.join(runtimeAssets, 'trayTemplate.png'),
    path.join(runtimeAssets, 'trayTemplate-active.png'),
  );
  await copyFile(
    path.join(runtimeAssets, 'trayTemplate@2x.png'),
    path.join(runtimeAssets, 'trayTemplate-active@2x.png'),
  );
  ...
  await copyFile(
    path.join(runtimeAssets, 'tray-icon.png'),
    path.join(runtimeAssets, 'tray-icon-active.png'),
  );
  ...
  await writeIcns(
    themeFileSource,
    'AgentSkinThemeFile',
    path.join(assets, 'branding', 'theme-file.icns'),
  );
