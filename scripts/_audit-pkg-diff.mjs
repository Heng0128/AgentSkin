...
for (const dir of dirs) {
  ...
  const cm = JSON.parse(
    fs.readFileSync(path.join(THEMES_DIR, dir, `${dir}.agentskin-theme/manifest.json`), 'utf8'),
  );
  ...
  const compCss = fs.readFileSync(
    path.join(THEMES_DIR, dir, `${dir}.agentskin-theme/assets/css/traework.css`),
    'utf8',
  );
  ...
  const sc = sm.colors || {};
  const cc = cm.colors || {};
  const _allKeys = new Set([...Object.keys(sc), ...Object.keys(cc)]).forEach((k) => {
  ...
  const _DEFAULT_ACCENT = '#9d8bff';
  const _DEFAULT_BG = '#201a40';
  const _DEFAULT_BLUE = 'rgba(157, 139, 255';

  if (compCss.includes(DEFAULT_SELECTION) && !srcPalette.includes(DEFAULT_SELECTION)) {
    console.log(
      `  \u26a0 DEFAULT LEAK: compiled CSS contains DEFAULT_TOKENS --agentskin-selection="${DEFAULT_SELECTION}" (not in source palette)`,
    );
  }
