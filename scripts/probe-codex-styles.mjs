...
const JS = `(() => {
  ...
  for (const s of Array.from(document.querySelectorAll('style'))) {
    ...
    if (/--agentskin-accent/.test(t) || /^agentskin-/i.test(s.id || '')) {
      const accentRules = [];
      for (const r of s.sheet ? s.sheet.cssRules : []) {
        if (/--agentskin-accent/.test(r.cssText || '')) accentRules.push(r.cssText);
      }
      out[s.id || '(none)'] = { len: t.length, accentRules };
    }
  }
  return out;
})()`;
