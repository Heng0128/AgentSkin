// Probe #9 DECISIVE: set tokens inline !important on html, read consumers in same tick.
...
const JS = `(() => {
  ...
  const set = (k) => base.style.setProperty(k, 'rgba(255,0,0,0.9)', 'important');
  ['--bg-bg-input','--bg-bg-base-secondary','--bg-bg-overlay-l1','--bg-bg-overlay-l2','--bg-bg-white','--vscode-icube--bg-bg-overlay-l1'].forEach(set);
  ...
  const read = (sel) => {
    ...
    return { sel, bg: cs.backgroundColor, bgImg: cs.backgroundImage };
  };
  const out = [
  ...
  // restore
  ['--bg-bg-input','--bg-bg-base-secondary','--bg-bg-overlay-l1','--bg-bg-overlay-l2','--bg-bg-white','--vscode-icube--bg-bg-overlay-l1'].forEach(k => base.style.removeProperty(k));
  return out;
})()`;
async function run(){
