...
| `../debug-tools/` | 一次性探针/调试脚本（`probe-*`、`apply-*`、`debug-*`、`cleanup-*`）。**调试资产，禁止删除**。 |
...
| `check-dependency-audit.mjs` | 依赖供应链审计：扫描 install/postinstall 脚本、传递依赖阈值、Snyk 漏洞（C9 扩展） |
...
| `agentskin-compiler.mjs` | 统一编译 CLI（λ S4）：build/verify/diagnose 三子命令，集成 sanitize（P0-2）与 specificity（P0-1）护栏，包装 `build-theme-package.mjs` |
