...
function countReferences(targetName, allFiles, sourceFile) {
  ...
  for (const file of allFiles) {
    ...
    if (file.endsWith('.test.ts') && content.includes('import')) {
      ...
      const importFromSource = content.includes(`from`) &&
        allFiles.find(f => f === sourceFile && content.includes(relative(dirname(file), sourceFile).replace(/\\/g, '/').replace('.ts', '')));
...
function findSimilarFiles(files, baseName) {
  const matches = [];
  for (const f of files) {
    const fBase = f.split('/').pop()?.replace(/\.(ts|tsx)$/, '');
...
function detectDuplicates(files) {
  ...
  for (const [file1, sigs1] of fileContents) {
    ...
    for (const [file2, sigs2] of fileContents) {
      ...
      if (common.length >= 2) {
        duplicates.push({
          files: [relative(srcDir, file1).replace(/\\/g, '/'), relative(srcDir, file2).replace(/\\/g, '/')],
...
const filteredDead = deadExports.filter(d => {
  ...
  if (['default', 'config', 'options', 'settings', 'types'].includes(d.name.toLowerCase())) return false;
  return true;
});
