...
function normalizedHex(val) {
  if (!val) return null;
  // 过滤明显非颜色的值
  if (/calc\(|var\(|oklch\(|color-mix|linear-gradient|rgba?\([^)]*\d\s*\)/.test(val) && !/^#[0-9a-fA-F]{3,8}$/.test(val) && !/^rgba?\(\d+,\s*\d+,\s*\d+/.test(val)) {
    // 仍尝试从 rgb() 提取
    if (val.startsWith('rgb')) {
      const rgba = parseRgba(val);
      if (rgba && !isNaN(rgba.r)) return rgbToHex(rgba.r, rgba.g, rgba.b);
    }
    return null;
  }
  // 过滤纯数字、非颜色关键字
  if (/^\d+(\.\d+)?(px|rem|em|%|s|ms)?$/.test(val)) return null;
  if (/^(inherit|initial|unset|none|auto|normal|transparent|currentColor|visible|hidden|block|flex|grid|inline|absolute|relative|fixed|sticky)$/i.test(val)) return null;
...
const RULES = {
  // 背景色类
  backgrounds: [
    /background/i, /surface/i, /bg-/i, /fill(?!-)/i,
    /canvas/i, /base(?!-)/i, /layer/i, /panel(?!-)/i,
    /card/i, /popover/i, /modal|dialog/i, /overlay/i,
    /backdrop/i, /container(?!-)/i, /layout/i,
    /main-surface/i, /editor-/i, /sidebar/i, /header/i,
    /menubar/i, /titlebar/i, /tab(?!le)/i, /toolbar/i,
    /notification|toast/i, /tooltip/i,
  ],
  // 文本色类
  text: [
    /text/i, /foreground/i, /fg(?!-)/i, /label/i,
    /title(?!-)/i, /heading/i, /paragraph/i,
    /placeholder/i, /muted/i, /description/i,
    /primary.*text/i, /secondary.*text/i,
  ],
  // 边框类
  borders: [
    /border/i, /divider/i, /separator/i, /outline/i,
    /stroke(?!-)/i,
  ],
  // 主题强调色
  accent: [
    /accent/i, /primary/i, /brand/i, /theme/i,
    /highlight/i, /selection/i, /focus/i,
    /link/i, /caret/i, /cursor(?!-)/i,
    /active/i, /pressed/i, /hover(?!board)/i,
    /checked/i, /selected(?!-)/i, /current/i,
    /step/i, /progress/i,
  ],
  // 语义状态色
  semantic: [
    /error/i, /danger/i, /danger/i,
    /warning/i, /warn/i,
    /success/i, /correct/i, /passed/i,
    /info/i, /notice/i,
    /disabled/i, /readonly/i, /inactive/i,
    /new/i, /modified/i, /added/i, /deleted/i, /removed/i,
    /conflict/i, /merged/i,
  ],
  // 阴影
  shadows: [
    /shadow/i, /elevation/i, /glow/i, /halo/i,
    /ring(?!er)/i,
  ],
  // 字体
  typography: [
  ...
  // 间距
  spacing: [
    /spacing/i, /gap/i, /margin/i, /padding/i,
  ...
  // 输入框
  inputs: [
    /input/i, /field/i, /editor/i, /textarea/i,
    /search/i,
  ],
  // 滚动条
  scrollbar: [
    /scroll/i, /scrollbar/i, /thumb/i, /track(?!ing)/i,
  ],
  // 代码块
  code: [
    /code/i, /mono/i, /syntax/i,
    /keyword/i, /string(?!-)/i, /comment/i,
    /function/i, /variable/i, /number/i, /type/i,
    /operator/i, /punctuation/i, /tag-attr/i,
  ],
  // 图标
  icon: [
    /icon/i, /svg/i, /fill-/i,
...
function extractDesignTokens(rootVars, lightVars, darkVars) {
  const result = {
    light: {},
    dark: {},
  };
  ...
  const tokenSearchPaths = {
    'background': ['--color-background', '--bg', '--background', '--color-bg', '--surface', '--color-surface', '--background-color', '--color-canvas', '--vscode-editor-background', '--color-token-main-surface-primary'],
    'foreground': ['--color-foreground', '--foreground', '--text', '--color-text', '--color-fg', '--color', '--vscode-foreground', '--color-token-primary'],
    'surface': ['--color-surface', '--surface', '--color-bg-elevated', '--color-card', '--color-panel', '--panel', '--card', '--color-container', '--color-token-secondary'],
    'border': ['--color-border', '--border', '--color-border-primary', '--border-color', '--color-divider', '--color-separator', '--vscode-panel-border'],
    'accent': ['--color-accent', '--accent', '--primary', '--color-primary', '--color-brand', '--brand', '--color-theme', '--vscode-focusBorder', '--color-token-accent'],
    'muted': ['--muted', '--color-muted', '--fg-muted', '--text-muted', '--color-text-secondary', '--vscode-descriptionForeground'],
    'error': ['--color-error', '--error', '--color-danger', '--danger', '--color-red', '--vscode-errorForeground'],
    'warning': ['--color-warning', '--warning', '--color-warn', '--color-yellow', '--color-orange', '--vscode-warningForeground'],
    'success': ['--color-success', '--success', '--color-correct', '--color-green', '--vscode-testing-iconPassed'],
...
function analyzeDomStructure(domTree) {
  ...
  function walk(node) {
    ...
    // 根据结构推断组件
    if (tag === 'input' || tag === 'textarea' || (node.c && node.c.includes('editor'))) {
...
function buildAgentProfile(agentName, rawData) {
  const { rootVariables, variables, categories, colorPalette, dom, computedSample, stats, meta, stylesheets } = rawData;
  ...
  for (const [scheme, samples] of Object.entries(computedSample)) {
    ...
    computedStyleSummary[scheme] = {
      ...
      topBgColors: Object.entries(bgColors).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([c, n]) => ({ color: c, count: n })),
      topTextColors: Object.entries(textColors).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([c, n]) => ({ color: c, count: n })),
      topFonts: Object.entries(fonts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([f, n]) => ({ font: f, count: n })),
      topRadii: Object.entries(radii).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([r, n]) => ({ radius: r, count: n })),
    };
  }
...
async function main() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(`  ✓ ${agentName}: tokens=${Object.keys(profile.tokens.core.light).length}, categories=${Object.keys(profile.tokenTree).length}, DOM components=${profile.dom.structure?.components?.length || 0}`);
    } catch (e) {
  ...
  const summary = {
    ...
    profiles: Object.fromEntries(
      Object.entries(profiles).map(([name, p]) => [name, {
        tokensLight: Object.keys(p.tokens.core.light).filter(k => p.tokens.core.light[k]).length,
