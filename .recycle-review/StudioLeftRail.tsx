...
export function StudioLeftRail({ t }: { t: UiMessages }) {
  ...
  return (
    <div
      className="overflow-y-auto border-r border-border px-3 pt-3 pb-3"
      style={{ background: 'var(--bg, var(--background))' }}
    >
      {/* Primary actions — New / Import pinned to top of rail */}
      <div className="flex gap-1 pb-2">
        <button
          type="button"
          onClick={() => setCreatingProject(true)}
          className="flex h-7 flex-1 items-center justify-center gap-1.5 border border-primary/40 bg-primary/10 px-2 font-mono text-[10px] font-bold uppercase transition-colors hover:bg-primary/20"
          style={{
        ...
        <button
          ...
          disabled={importing}
          className="flex h-7 flex-1 items-center justify-center gap-1.5 border border-border bg-muted px-2 font-mono text-[10px] font-bold uppercase transition-colors hover:bg-accent disabled:opacity-40"
        ...
        >
          ...
          <div className="flex gap-1.5 pt-1">
            <button
              ...
              className="h-6 flex-1 border border-border bg-primary px-2 font-mono text-[10px] font-bold uppercase text-primary-foreground"
            ...
            <button
              ...
              className="h-6 border border-border bg-muted px-2 font-mono text-[10px] uppercase"
              style={{ letterSpacing: '0.06em', borderRadius: 'var(--radius)' }}
            >
              {t.cancel}
            </button>
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className="bg-primary px-1.5 py-0.5 font-mono text-[10px] font-bold uppercase text-primary-foreground"
                      style={{ borderRadius: 'var(--radius)' }}
                    >
                      {t.studioProjectSave}
                    </button>
                ...
                >
                  ...
                  <div className="mt-0.5 flex items-center gap-1">
                    {p.hasSnapshot ? (
                      <Badge
                        variant="outline"
                        className="gap-1 px-1 py-0.5 font-mono text-[10px] font-bold uppercase text-cr-success border-cr-success/45"
                      >
                      ...
                      <Badge
                        variant="outline"
                        className="px-1 py-0.5 font-mono text-[10px] uppercase text-muted-foreground border-border"
                      >
                        {t.studioProjectIdle}
                      </Badge>
                    )}
                    {p.exportedDir && (
                      <Badge
                        variant="accent"
                        className="px-1 py-0.5 font-mono text-[10px] font-bold uppercase"
                      >
            ...
            {installedThemes.length === 0 ? (
              ...
              installedThemes.map((theme) => (
                <button
                  ...
                  className="flex w-full items-center gap-1.5 rounded-md px-1.5 py-1 text-left transition-colors hover:bg-accent disabled:opacity-40"
                  title={
                    activeProjectId
                ...
                >
                  {theme.icon ? (
                    <img src={theme.icon} alt="" className="size-3.5 shrink-0 rounded-md" />
                  ) : (
                    <span
      ...
      <div className="mt-4 space-y-2 border-t border-border pt-3">
        ...
        <button
          ...
          className="flex w-full items-center justify-between border border-border px-2 py-1.5 font-mono text-[10px] uppercase"
          style={{
            letterSpacing: '0.06em',
            borderRadius: 'var(--radius)',
