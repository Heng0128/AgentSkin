# traework 脆弱性分级（sdk-fragility）

> 与 [architecture.md](./architecture.md) 配套。依赖点按[升级崩溃概率]分级；
> 动态层数据来自 `agents-raw-data/traework-full-extract.json`（CDP 全量快照）。

## 1. 依赖点分级

| 等级 | 依赖点 | 崩溃概率 | 说明 |
|------|--------|---------|------|
| Low | `vscode-file://vscode-app` 渲染面身份 | Unlikely | scheme 稳定，决定 CDP 暴露面 |
...
| High | 本次快照数据质量 | Often | CORS 阻断或 DOM 截断 → 锚点/变量不可信 |
...
- [ ] 重新跑 CDP 全量快照：`node scripts/cdp-full-extract.mjs --agent traework`
- [ ] 与上一版 diff：`node scripts/snapshot-compare.mjs <旧> <新> --out docs/apps/traework/raw/upgrade-diff.md`
