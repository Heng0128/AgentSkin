# workbuddy 脆弱性分级（sdk-fragility）

> 与 [architecture.md](./architecture.md) 配套。依赖点按[升级崩溃概率]分级；
> 动态层数据来自 `agents-raw-data/workbuddy-full-extract.json`（CDP 全量快照）。
...
- [ ] 重新跑 CDP 全量快照：`node scripts/cdp-full-extract.mjs --agent workbuddy`
- [ ] 与上一版 diff：`node scripts/snapshot-compare.mjs <旧> <新> --out docs/apps/workbuddy/raw/upgrade-diff.md`
