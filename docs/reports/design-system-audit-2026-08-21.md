...
- 要么变量基线应降回 2px（Swiss 锐利风格，历史方案，未采纳）
...
### 2.4 缩放/滑入动画违规（违反 Quiet Workbench 克制原则）

Quiet Workbench 设计原则明确禁止缩放和滑入动画。当前发现 12 处违规：
...
### 方案 A：「Swiss 纯粹主义」（未采纳历史方案）

**核心理念**: 回归 Swiss/International 设计本源，2px 锐利圆角，严格克制。（未采纳）
...
#### 牺牲项
- 放弃 Swiss 纯粹 2px 方案（6px vs 2px，最终选择 Quiet Workbench 灵活档位）
...
| 维度 (权重) | 方案 A (Swiss 纯粹，未采纳) | 方案 B (精准校准) | 方案 C (系统重构) |
|-------------|---------------------|-------------------|-------------------|
| 业务根治 (20%) | 9 | 8 | 9 |
| 场景兼容 (15%) | 6 | 9 | 7 |
| 故障安全 (15%) | 6 | 9 | 6 |
...
#### 方案 A — Swiss 纯粹主义（7.45 分，未采纳）

| 维度 | 得分 | 理由 |
|------|------|------|
| 业务根治 | 9 | 彻底解决圆角哲学矛盾，回归 Swiss 本源（历史方案评估） |
| 场景兼容 | 6 | 6px → 2px 变化影响全部 50+ 组件，用户感知明显 |
| 故障安全 | 6 | 改动面大，回滚需还原 50+ 文件 |
| 工程契约 | 8 | 文档与代码可完全同步 |
| 可工程化 | 7 | C6 脚本需同步更新允许集 |
| 架构一致性 | 9 | 严格对齐 Swiss 设计原则（历史方案评估） |
...
| 牺牲项 | 说明 | 补偿措施 |
|--------|------|----------|
| 圆角策略 | 放弃 Swiss 2px 强制，采用 Quiet Workbench 灵活档位 | 文档记录"Quiet Workbench 圆角档位 (0/2/4/8px)" |
| 部分 P2 微动画 | 保留 translate-y-px 等微动画 | 限制在 `:active` 状态，符合 Quiet Workbench 克制原则 |
...
| 风险 | 等级 | 概率 | 影响 | 缓解措施 |
|------|------|------|------|----------|
| 圆角档位决策 | 中 | 高 | 中 | 采用 Quiet Workbench 灵活档位 (0/2/4/8px)，用户可自由选择 |
