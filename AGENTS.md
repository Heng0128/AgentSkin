...
8. **禁止根目录路径污染**：命令输出（typecheck/lint/test/check 的 txt/log）不得落根目录；确需保存统一放 `test-output/`。文档按类型归入 `docs/`、`docs/reports/`、`docs/rfc/`。脚本新增须登记 `scripts/INDEX.md`。详见 CONTRIBUTING.md「目录与文件规范」
