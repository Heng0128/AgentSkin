// Probe: verify which theme-layer tokens actually exist in the RUNNING WorkBuddy.
// Lightweight: only queries the generated theme's --cb-*/--wb-* token list via
// getComputedStyle on a few scopes — far smaller than the full extract that
// times out on VS-Code-family renderers. Reusable debug asset.
...
const gen = [...new Set([...css.matchAll(/(--(?:cb|wb)-[a-z0-9-]*):/g)].map(m => m[1]))];
...
async function run() {
  ...
  const js = `(() => {
    const T = ${T};
    const scopes = ['html', 'body', '#root', '[data-application-name="workbuddy"]', '.app-shell', '[class*="app-shell"]'];
    ...
    return { total: T.length, present: present.length, missing: missing.length, missingList: missing, sample: Object.fromEntries(present.slice(0, 8).map(t => [t, found[t].samples[0]])) };
  })()`;
