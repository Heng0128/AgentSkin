...
const JS = `(() => {
  ...
  const t = our.textContent || '';
  return {
    present: true,
    textLen: t.length,
    head: t.slice(0, 150),
    headHasRoot: /:root/.test(t.slice(0, 150)),
    hasRootBlock: /:root\\s*\\{/.test(t),
    hasAccentDecl: /--agentskin-accent\\s*:/.test(t),
    accentDeclHead: (t.match(/--agentskin-accent\\s*:[^;]*/) || ['(none)'])[0],
    // how many <style> tags present and their ids+len
    styles: Array.from(document.querySelectorAll('style')).map(s => ({ id: s.id || '(none)', len: (s.textContent || '').length })).slice(0, 10),
  };
})()`;
async function run() {
