...
import { wrapCatalog, type MainContext } from '../main-context';
...
import { isAgentId, type ApplyRequest } from '../../shared/types';
...
const THEME_PACKAGE_EXTENSIONS = ['agenttheme', 'agentskin-theme', 'codedrobe-theme', 'codex-theme'];

export function registerThemeIpc(deps: MainContext, updateTrayMenu: () => Promise<void>): void {
  // --- Catalog reads ---
  ipcMain.handle(IpcChannel.THEME_LIST, async () => wrapCatalog(await deps.themeCatalog.listThemes()));
  ...
  ipcMain.handle(IpcChannel.THEME_IMPORT_BYTES, async (_event, bytes: unknown, suggestedId: unknown) => {
  ...
  ipcMain.handle(IpcChannel.THEME_EXPORT, async (_event, themeId: unknown) => {
    if (typeof themeId !== 'string' || !themeId) throw new Error('Invalid theme id.');
    const copy = getMainMessages();
