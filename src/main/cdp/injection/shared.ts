 ...
 * # injection/shared
 *
 * Shared helpers and types used by both theme injection strategies
 * ({@link ./cdp-strategy} and {@link ./engine-strategy}).
 *
 * Contents:
 *   - **Types**: `ThemeVerification` (computed-style read-back after injection)
 *   - **Hero blob injection**: `transferHeroBase64`, `injectHeroBlob`,
 *     `injectHeroFromDataUrl` — converts local hero images to in-memory Blob
 *     URLs, bypassing file:// CSP restrictions (critical for Doubao).
 *   - **CSS injection**: `injectCssAdopted` (unnamed owned sheet),
 *     `injectCssLayer` (named layer for multi-layer engine architecture).
 *   - **Verification**: `verifyTheme` — reads back computed styles to confirm
 *     the theme actually took effect.
 *   - **Utilities**: `delay`.
 */
 ...
 * the same chunked-transfer strategy as {@link injectVideoWallpaperByBase64}:
