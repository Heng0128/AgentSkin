		...
		const value = await Promise.race([
			...
			new Promise<never>((_, reject) => {
				const timer = setTimeout(() => {
					reject(
						new Error(
							`Boot step "${label}" timed out after ${STEP_TIMEOUT_MS}ms`,
						),
					);
				}, STEP_TIMEOUT_MS);
...
export async function runBootSequence(deps: BootDeps): Promise<BootResult> {
	...
	if (process.platform === "darwin" && !app.isPackaged) {
		const dockIcon = nativeImage.createFromPath(
			path.join(brandingRoot(), "icon.png"),
		);
	...
	const localeResult = await runStep(
		...
		async () => {
			...
			ctx.locale = await loadLocalePreference(
				ctx.userDataRoot,
				app.getLocale(),
			);
	...
	const settingsResult = await runStep(
		...
		10,
		async () => {
			ctx.settings = new SettingsService(
				path.join(ctx.userDataRoot, "settings.json"),
			);
	...
	const catalogResult = await runStep(
		...
		async () => {
			...
			const installedSnapshots = new Map(
				bootThemes.map((t) => [
					t.id,
					{ version: t.version, contentHash: t.contentHash },
				]),
			);
			try {
				...
				sendLog(
					`[boot] theme seeding partial failure, continuing: ${toMessage(error)}`,
				);
			}
