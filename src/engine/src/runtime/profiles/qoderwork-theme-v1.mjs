...
function runtime({ theme, imageDataUrls = {}, imageUrls = {}, artDataUrl, artUrl = imageUrls.hero ?? artDataUrl }) {
  const CHROME_ID = "agentskin-qoderwork-skin-chrome";
  const copy = {
  ...
  const ensure = () => {
    ...
    root.classList.add("agentskin-qoderwork-skin");
  ...
  const cleanup = () => {
    const root = document.documentElement;
    root?.classList.remove("agentskin-qoderwork-skin");
  ...
  const verify = () => {
    ...
    const missing = [];
    if (!root?.classList.contains("agentskin-qoderwork-skin")) {
      missing.push({ name: "root-class", selectors: ["html.agentskin-qoderwork-skin"] });
    }
    if (!chrome) missing.push({ name: "chrome-layer", selectors: ["#agentskin-qoderwork-skin-chrome"] });
    if (chrome && getComputedStyle(chrome).pointerEvents !== "none") {
      missing.push({ name: "noninteractive-chrome", selectors: ["#agentskin-qoderwork-skin-chrome { pointer-events: none }"] });
    }
    return {
      id: "qoderwork-theme-v1",
      pass: missing.length === 0,
      missing,
      rootClassPresent: Boolean(root?.classList.contains("agentskin-qoderwork-skin")),
...
function cleanup() {
  ...
  document.getElementById("agentskin-qoderwork-skin-chrome")?.remove();
}
