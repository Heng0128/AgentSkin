 ...
 * to collect selector + declaration pairs. Handles `@layer` blocks by
 * recursively extracting inner rules and marking them as `inLayer: true`.
 */
function extractRules(css: string): CssRule[] {
  ...
  while (i < len) {
    ...
    const inner = stripped.slice(braceOpen + 1, j - 1);

    if (selector.startsWith('@layer')) {
      // Recursively extract rules from inside the layer block.
      const innerRules = extractRules(inner);
      for (const rule of innerRules) {
        rule.inLayer = true;
        rules.push(rule);
      }
    } else if (selector && !selector.startsWith('@')) {
      rules.push({ selector, declarations: inner, inLayer: false });
    }
...
export function validateSpecificity(css: string, profile: SpecificityProfile): SpecificityReport {
  ...
  // Rules already inside @layer are excluded — cascade layers defuse
  // specificity wars, so their selectors need not respect the ceiling.
...
export function guardSpecificity(
  ...
  // Return the original report (pre-fix) so callers can see what was violated.
  // The guarded CSS is the auto-fixed output; callers may re-validate if needed.
