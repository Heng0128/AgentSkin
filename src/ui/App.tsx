      ...
      >
        <div className={cn('grid min-h-0 overflow-hidden', 'grid-cols-[56px_1fr]')}>
          ...
          <main className="flex min-h-0 flex-col overflow-hidden">
            ...
            <div className="scroll-fade-y min-h-0 flex-1 overflow-y-auto overflow-x-hidden">
              <div
                key={controller.route}
                className={cn('page-enter h-full')}
                style={{
        ...
        {controller.toasts.map((toast) => (
          <div
            key={toast.id}
            className={cn(
              'as-toast-enter fixed bottom-10 left-1/2 z-[var(--z-toast)] -translate-x-1/2 rounded-lg border px-4 py-2.5 text-[12px] font-medium shadow-lg backdrop-blur-md',
