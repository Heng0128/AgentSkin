 ...
 * Structure:
 *   Brand (top)
 *   Navigation — grouped section labels, active item gets a primary-colored
 *     left rule
...
export function Sidebar() {
  ...
  // Grouping — mirrors the A.html reference nav (CORE / MANAGE / SYSTEM).
  ...
  return (
    ...
    >
      {/* Brand — clicking the logo toggles the collapsed state (the brand
          mark doubles as the collapse control; the footer button remains as
      ...
      <button
        ...
        className={cn(
          'flex items-center gap-2 px-4 py-2 shrink-0 cursor-pointer bg-transparent border-0 text-left transition-colors duration-fast',
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden pt-2 pb-2">
        {navGroups.map((group) => (
          <div key={group.label} className="mb-1">
            {!collapsed && (
              <div className="px-4 pt-1 pb-1 text-[11px] font-medium text-[var(--muted-foreground)]">
                {group.label}
              </div>
            )}
      ...
      {/* Footer */}
      <div className="shrink-0 border-t border-[var(--border)] p-2 flex flex-col gap-1">
        ...
        <button
          ...
          className={cn(
            'flex h-8 w-full items-center justify-center gap-2 rounded-md border border-dashed border-[var(--border-strong)] bg-transparent text-[11px] font-medium text-[var(--muted-foreground)] cursor-pointer whitespace-nowrap transition-colors duration-fast',
