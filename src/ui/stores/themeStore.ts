...
export const useThemeStore = create<ThemeState>((set, get) => {
	...
	offFileImported = api.onFileImported(async (result) => {
		...
		useNotificationStore
			.getState()
			.showToast(currentT().importedTheme(result.theme.displayName));
	});
	offFileImportConfirm = api.onFileImportConfirm(
		useDialogStore.getState().setFileImportPrompt,
	);
	offFileImportFailed = api.onFileImportFailed((message) => {
		useNotificationStore
			.getState()
			.showToast(message || currentT().actionFailed, "destructive");
	});
	...
	return {
		...
		applyToApp: async (themeId, themeName, appId, options = {}) => {
			...
			switch (outcome.kind) {
				...
				case "success": {
					...
					// Activate the theme's video wallpaper, if it has one.
					const theme = get().installedById(themeId);
					if (theme?.wallpaper) {
						void useWallpaperStore
							.getState()
							.activateThemeWallpaper(themeId, theme.wallpaper.workshopId)
							.catch(() => undefined);
					}
					return true;
				}
				case "unknown-status":
					...
					useNotificationStore
						.getState()
						.showToast(
							`Theme apply returned unexpected status: ${outcome.status}`,
							"destructive",
						);
					return false;
			}
		...
		restoreApp: async (appId) => {
			const t = currentT();
			const result = await withBusy(`restore:${appId}`, () =>
				api.restoreApp(appId),
			);
			if (!result) return;
			// Same as applyToApp: refresh authoritative status.
			await useStatusStore.getState().refreshStatus();
			const appName =
				result.apps.find((a) => a.appId === appId)?.displayName ??
				APP_META[appId]?.name ??
				appId;
		...
		restoreAll: async () => {
			...
			const results = await Promise.all(
				targets.map(async (app) => {
					const result = await withBusy(`restore:${app.appId}`, () =>
						api.restoreApp(app.appId),
					);
		...
		confirmFileImport: async () => {
			const { fileImportPrompt, setFileImportPrompt } =
			...
			const didAcquire = await withImportLock(targetPath, async () => {
				const result = await withBusy("import", () =>
					api.importThemeFromPath(targetPath),
				);
				if (!result) return;
				await get().refreshThemes();
				useNotificationStore
		...
		dropThemeFiles: (files) => {
			for (const file of files) {
				if (!/\.(agenttheme|agentskin-theme|codex-theme)$/.test(file.name))
					continue;
				const path = api.getPathForFile(file);
				if (path)
					void api
						.openThemeFile(path)
						.catch(useNotificationStore.getState().fail);
			}
		},

		exportTheme: async (themeId) => {
			const t = currentT();
			const result = await withBusy(`export:${themeId}`, () =>
				api.exportTheme(themeId),
			);
			if (result && !result.canceled)
				useNotificationStore.getState().showToast(t.packageExported);
		},

		createBundle: async (themeId) => {
			const t = currentT();
			const result = await withBusy(`bundle:${themeId}`, () =>
				api.createBundle(themeId),
			);
			if (result && !result.canceled)
				useNotificationStore.getState().showToast(t.bundleExported);
		},
		...
		confirmDelete: async () => {
			...
			const result = await withBusy(`delete:${theme.id}`, () =>
			...
			set((current) =>
				current.selection?.kind === "installed" &&
				current.selection.theme.id === theme.id
					? { selection: null }
					: {},
			);
...
async function withBusy<T>(
	...
	const t = currentT();
	if (busyKeys.has(key)) {
		const msg = (t as Record<string, unknown>).busyOperationInProgress as
			| string
			| undefined;
		useNotificationStore
			.getState()
	...
	while (busyKeys.size >= MAX_CONCURRENCY) {
		if (elapsed >= MAX_BUSY_WAIT_MS) {
			const msg = (t as Record<string, unknown>).busyTimeout as
				| string
				| undefined;
			useNotificationStore
				.getState()
				.fail(
					msg ?? "Timed out waiting for a free operation slot — try again.",
				);
			return undefined;
		}
